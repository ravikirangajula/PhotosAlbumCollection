...
//  PhotoALbum
//
//  Created by Gajula RaviKiran on 07/02/2021.
//

import Foundation
import UIKit

class MosaicGridLayout: UICollectionViewLayout {
    ...
    fileprivate func collectionView(_ collectionView: UICollectionView, sizeForPhotoAtIndexPath indexPath:IndexPath) -> CGRect {
        ...
        if indexPath.row == 0 {
            ...
            return CGRect(x: (width * 2), y: ((indexPath.row == 2) ? width : 0), width: width, height: height)
        } else {
            
            var drawFromRight = true
            ...
            switch columnValue {
            case 0:
                return CGRect(x: (drawFromRight ? CGFloat((indexPath.row % 3)) * width : (2 * width)), y: 0, width: width, height: height)
                
            case 1:
                return CGRect(x: CGFloat((indexPath.row % 3)) * width, y: 0, width: width, height: height)
                
            case 2:
                return CGRect(x: (drawFromRight ? CGFloat((indexPath.row % 3)) * width : 0), y: 0, width: width, height: height)
